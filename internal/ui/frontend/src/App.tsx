...
function sectionForIssue(path: string): SectionID {
  if (path === "output_path") return "build";
...
export function App() {
  ...
  const [outputMode, setOutputMode] = useState<OutputMode>("cache");
  ...
  const applyOpenResponse = (response: Awaited<ReturnType<typeof api.open>>, nextPath: string, resetSession: boolean) => {
    ...
    if (resetSession) { setBuild(null); setLastSuccess(null); setOutputMode("cache"); setCustomOutputPath(""); setOverwritePath(null); }
  };
  ...
  const startBuild = async (confirmOverwrite = false, confirmedPath?: string) => {
    const requestedOutput = confirmedPath ?? (outputMode === "custom" ? customOutputPath.trim() : "");
    if (outputMode === "custom" && !requestedOutput) {
      const issue: ValidationIssue = { path: "output_path", code: "invalid_output_path", message: "请输入绝对输出路径。", severity: "error" };
      setValidationIssues((current) => [...current.filter((item) => item.path !== "output_path"), issue]);
      setNotice({ kind: "error", message: issue.message });
    ...
    try {
      setBuild(await api.startBuild(activePath, requestedOutput, confirmOverwrite));
      setOverwritePath(null); setValidationIssues([]); setNotice(null);
    } catch (error) {
      if (error instanceof APIError && error.code === "output_overwrite_confirmation_required") {
        setOverwritePath(error.outputPath ?? requestedOutput); setValidationIssues([]); setNotice(null);
      } else {
        setOverwritePath(null); showError(error, "无法启动构建。");
      }
      setBusy(false);
    }
  };
  ...
  const renderSection = () => {
    ...
    if (activeSection === "build") return <Section title="构建" description="选择当前页面会话的输出位置，再查看结构化阶段、warning 与最终路径。"><div className="build-output-settings"><div><strong>输出目录</strong><p>默认写入仓库外缓存；自定义路径只影响当前项目会话，不写入 `.repolens.yml`。</p></div><div className="output-mode" role="group" aria-label="输出模式"><button type="button" aria-pressed={outputMode === "cache"} disabled={busy} onClick={() => { setOutputMode("cache"); clearIssue("output_path"); }}>缓存目录</button><button type="button" aria-pressed={outputMode === "custom"} disabled={busy} onClick={() => setOutputMode("custom")}>自定义目录</button></div>{outputMode === "custom" ? <Field label="绝对输出路径" hint="允许不存在目录、空目录或 repolens 自有旧产物。" error={fieldError("output_path")} errorID={errorID("output_path")}><TextInput name="output_path" aria-invalid={Boolean(fieldError("output_path"))} aria-describedby={fieldError("output_path") ? errorID("output_path") : undefined} value={customOutputPath} placeholder="/Users/name/Sites/repolens-dist" disabled={busy} onChange={(event) => { setCustomOutputPath(event.target.value); clearIssue("output_path"); }} /></Field> : <Alert kind="info">缓存路径由 repolens 管理，适合本地检查且不会写入仓库。</Alert>}</div><BuildSummary build={build} lastSuccess={lastSuccess} /></Section>;
  ...
  return <div className="app-shell" aria-busy={busy}><WorkspaceHeader path={activePath} /><main className="page">
    {!settings ? <section className="open-panel"><div><span className="eyebrow">本地工作树</span><h1>安全地配置并构建仓库</h1><p>输入绝对路径。仓库内容只在本机读取，构建默认写入仓库外缓存。</p></div><div className="path-row"><Field label="仓库绝对路径" error={pathError ?? undefined} errorID="project-path-error"><TextInput id="project-path" aria-label="仓库绝对路径" aria-invalid={Boolean(pathError)} aria-describedby={pathError ? "project-path-error" : undefined} value={path} onChange={(e) => { setPath(e.target.value); setPathError(null); }} placeholder="/Users/name/Projects/repository" disabled={busy} /></Field><Button variant="primary" onClick={openProject} disabled={busy}>打开项目</Button></div></section> : <>
      ...
      <StickyActions state={dirty ? "请校验并预览写入" : "配置与磁盘一致"}><Button disabled={busy} onClick={validate}>校验配置</Button><Button ref={prepareButtonRef} variant="primary" disabled={busy || !dirty} onClick={prepare}>预览写入 diff</Button><Button ref={buildButtonRef} variant="success" disabled={busy || dirty} onClick={() => startBuild()}>开始构建</Button></StickyActions>
    </>}
  ...
  <AlertDialog.Root open={Boolean(overwritePath)} onOpenChange={(open) => { if (!open && !busy) { setOverwritePath(null); window.requestAnimationFrame(() => { const output = document.querySelector<HTMLInputElement>('[name="output_path"]'); (output ?? buildButtonRef.current)?.focus(); }); } }}><AlertDialog.Portal><AlertDialog.Backdrop className="dialog-backdrop" /><AlertDialog.Viewport className="dialog-viewport"><AlertDialog.Popup className="dialog-popup overwrite-dialog"><AlertDialog.Title className="dialog-title">替换 repolens 的旧构建产物？</AlertDialog.Title><AlertDialog.Description className="dialog-description">目标目录包含 `.repolens-build` 哨兵。确认后会在同级临时目录完成构建，发布前再次检查所有权，再原子替换当前目录。</AlertDialog.Description><code className="target-path">{overwritePath}</code><Alert kind="success">构建或发布失败时保留最近一次成功产物。</Alert><div className="button-row end"><AlertDialog.Close className="button button-secondary" disabled={busy}>返回修改</AlertDialog.Close><Button variant="danger" disabled={busy} onClick={() => startBuild(true, overwritePath ?? undefined)}>确认替换并构建</Button></div></AlertDialog.Popup></AlertDialog.Viewport></AlertDialog.Portal></AlertDialog.Root>
  </div>;
}

function BuildSummary({ build, lastSuccess }: { build: BuildResponse | null; lastSuccess: BuildResponse | null }) {
  return <div className="build-state"><div className="build-state-heading"><strong>{build ? build.stage : "尚未构建"}</strong>{build ? <Badge kind={build.output_mode === "custom" ? "repository" : "default-value"}>{build.output_mode === "custom" ? "自定义目录" : "缓存目录"}</Badge> : null}</div>{build?.stats ? <dl><div><dt>文件</dt><dd>{build.stats.Files}</dd></div><div><dt>页面</dt><dd>{build.stats.Pages}</dd></div><div><dt>耗时</dt><dd>{Math.round(build.stats.Duration / 1_000_000)}ms</dd></div></dl> : null}{build?.output_path ? <code>{build.output_path}</code> : null}{build?.warnings?.length ? <ul>{build.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul> : null}{build?.error ? <Alert kind="error">{build.error}</Alert> : null}{build?.stage === "failed" && lastSuccess?.output_path ? <Alert kind="success">最近一次成功构建仍可用：<code>{lastSuccess.output_path}</code></Alert> : null}</div>;
}
