...
Requirements: Go ≥ 1.25 and `git` on PATH (repolens shells out to the system git).
...
- **New dependencies need justification.** Direct dependencies stay in the single digits; candidates must be mainstream and actively maintained. No Node toolchain, ever.
...
3. Quality gate must pass locally and in CI: `gofmt -l .` (no output), `go vet ./...`, `go test ./...`, `go build ./...`.
