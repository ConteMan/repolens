...
- Skill 安装与更新：skill 内嵌进二进制，`repolens skill install / list / update` 按 Agent Skills 开放标准探测各产品的 skill 目录并离线保持副本最新（[019](specs/019-skill-install.md)）

### Out（v2 及以后，实现前需先修订本文档）

- 全文搜索（v1.x 仅文件名＋标题级；全文仍预定 pagefind 后处理方案）
- 正文术语的启发式自动识别与自动加注（ADR-007 决策 2 明确排除；如提供只能是面向人的建议输出，不进入渲染）
- 全站术语索引页面（涉及新的 URL 约定，需先修订 ADR-001）
- 格式导出（PDF / docx / epub）
...
| M12 Skill 分发 | skill 内嵌二进制，安装/列出/更新子命令与多 Agent 目录适配 | [019](specs/README.md) | 干净仓库一条命令装好、重复执行幂等、本地修改不被覆盖、离线更新生效、质量门禁全绿 |
