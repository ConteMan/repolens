...
├── index.json                 # 机器可读站点索引
├── _assets/                   # 站点自身的 CSS / JS / 字体与快照元数据（非仓库内容）
└── robots.txt
```
...
- 每次成功构建生成一个不透明快照 ID，浏览页与稳定同源资源 `_assets/snapshot.json` 同时携带该 ID。薄增强层仅在页面可见且在线时按保守周期探测，并在页面重新可见或恢复联网时立即探测；请求使用相对 URL 并显式绕过缓存。检测到新快照后显示可访问、非阻塞的更新提示，由访问者主动重新加载，不自动刷新、不抢焦点；网络失败静默降级；
