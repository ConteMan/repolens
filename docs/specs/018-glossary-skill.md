...
1. **位置与形态**：skill 存放于仓库顶层 `skills/<name>/`，主文件为 `SKILL.md`——YAML front matter 提供 `name`、`description`、规范源指向，正文为纯 Markdown 操作规范。术语标注 skill 为 `skills/glossary/`。
...
4. **自包含**：skill 需要在 repolens 仓库之外独立使用，因此复述了 spec 017 中作者可见的那部分格式约定（字段表、语法、配置片段）。front matter 显式声明规范源与版本；skill 与 spec 冲突时以 spec 为准。
5. **固定样例**：`skills/glossary/example/` 是一个最小的完整仓库片段，包含 `.repolens.yml`、公共术语库条目与一篇使用了标注、front matter 覆盖和私有术语的文档。它同时是文档示例与 CI fixture。
6. **一致性检查**：CI 对 `skills/glossary/example/` 执行构建，断言构建成功且产物包含术语表小节。spec 017 的格式若变更而 skill 未同步，该检查失败。检查随 spec 017 实现一并启用；在此之前 example 仅作为文档样例存在。
7. **不进入二进制**：skill 不 `go:embed`、不随发布二进制分发、不参与站点构建输出。它随源码仓库分发，使用者复制或引用文件路径即可。
...
```
skills/glossary/
├── SKILL.md                              # front matter 含 name / description / spec / spec_version
└── example/
...
- `skills/glossary/SKILL.md` 存在且 front matter 含 `name`、`description`、`spec`、`spec_version`；
- 全文不含具体 Agent 产品的工具名与加载机制依赖（`skills/README.md` 的接入方式说明除外）；
- SKILL.md 中的字段表、语法、配置片段与 spec 017 一致；
