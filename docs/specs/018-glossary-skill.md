...
6. **固定样例**：`skills/repolens-glossary/example/` 是一个最小的完整仓库片段，包含 `.repolens.yml`、公共术语库条目与一篇使用了标注、front matter 覆盖和私有术语的文档。它同时是文档示例与 CI fixture。
7. **一致性检查**：自动化测试构建 `skills/repolens-glossary/example/` 并断言 skill 所教的每一项写法都在产物中真实生效。检查随 spec 017 实现一并启用。
...
本 spec 不引入 Go 接口。契约是 `skills/repolens-glossary/example/` 的内容约定：

```
skills/repolens-glossary/
...
- `skills/repolens-glossary/SKILL.md` 存在且 front matter 仅含 `name` 与 `description`；
